<?php

class RequestKey {
  //USER
  public static $USER_ID              = 'user-id';
  public static $USER_NAME            = 'user-name';
  public static $USER_USERNAME        = 'user-username';
  public static $USER_PASSWORD        = 'user-password';
  public static $USER_LEVEL           = 'user-level';


  //PLACE
  public static $PLACE_ID             = 'place-id';
  public static $PLACE_NAME           = 'place-name';
  public static $PLACE_LOCATION       = 'place-location';
  public static $PLACE_CATEGORY       = 'place-category';

  //MAJID
  public static $MASJID_ID            = 'masjid-id';
  public static $MASJID_PLACE_ID      = 'masjid-place-id';
  public static $MASJID_NAME          = 'masjid-name';
  public static $MASJID_HISTORY       = 'masjid-history';

  //KAJIAN
  public static $KAJIAN_ID            = 'kajian-id';
  public static $KAJIAN_MASJID_ID     = 'kajian-masjid-id';
  public static $KAJIAN_DATE          = 'kajian-date';
  public static $KAJIAN_TIME          = 'kajian-time';
  public static $KAJIAN_TITLE         = 'kajian-title';
  public static $KAJIAN_DESCRIPTION   = 'kajian-description';
  public static $KAJIAN_SPEAKER       = 'kajian-speaker';

  //JUMAT
  public static $JUMAT_ID             = 'jumat-id';
  public static $JUMAT_MASJID_ID      = 'jumat-masjid-id';
  public static $JUMAT_DATE           = 'jumat-date';
  public static $JUMAT_IMAM           = 'jumat-imam';

  //KEGIATAN
  public static $KEGIATAN_ID          = 'kegiatan-id';
  public static $KEGIATAN_MASJID_ID   = 'kegiatan-masjid-id';
  public static $KEGIATAN_DATE        = 'kegiatan-date';
  public static $KEGIATAN_TIME        = 'kegiatan-time';
  public static $KEGIATAN_TITLE       = 'kegiatan-title';
  public static $KEGIATAN_DESCRIPTION = 'kegiatan-description';

  //FAMILYkegiatan
  public static $FAMILY_ID            = 'family-id';
  public static $FAMILY_PLACE_ID      = 'family-place-id';
  public static $FAMILY_NAME          = 'family-name';
  public static $FAMILY_STATUS        = 'family-status';
  public static $FAMILY_RELIGION      = 'family-religion';
  public static $FAMILY_AGE        = 'family-age';
  public static $FAMILY_GENDER        = 'family-gender';
  public static $FAMILY_BORN_PLACE    = 'family-born-place';
  public static $FAMILY_BORN_DATE     = 'family-born-date';
  public static $FAMILY_EDUCATION     = 'family-education';
  public static $FAMILY_SALARY        = 'family-salary';
  public static $FAMILY_BLOOD         = 'family-blood';

  //KEIMANAN
  public static $KEIMANAN_ID          = 'keimanan-id';
  public static $KEIMANAN_FAMILY_ID   = 'keimanan-family-id';
  public static $KEIMANAN_SHOLAT      = 'keimanan-sholat';
  public static $KEIMANAN_MENGAJI     = 'keimanan-mengaji';



 }

?>
